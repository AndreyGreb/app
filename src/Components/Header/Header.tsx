...
interface IHeader {
  openFormSignIn: (value:any) => void,
  colorHeader: string
}

const Header = ({openFormSignIn, colorHeader}:IHeader) => {
  return (
      ...
      <header className={`${style.header} ${style[colorHeader]}`}>
        <nav className={style.navbar}>
          ...
          <Link to="/" className={style.logo}>MODNIKKY</Link>
          <div className={style.navbar__right}>
            ...
            <div onClick={openFormSignIn} onKeyDown={openFormSignIn} role="presentation">SIGN IN</div>
            <div className={style.navbar__right__bag}>
              <div>BAG</div>
              <span>(2)</span>
            </div>
            <div>
